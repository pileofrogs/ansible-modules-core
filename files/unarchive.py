...
options:
  ...
  extra_opts:
    description:
      - Additional arguments to pass to the unarchive tool when decompressing an archive.
    required: false
    default: null
...
class ZipArchive(object):
    ...
    def __init__(self, src, dest, extra_opts, module):
        self.src = src
        self.dest = dest
    ...
    def unarchive(self):
        cmd = '%s %s -o "%s" -d "%s"' % (self.cmd_path, self.extra_opts, self.src, self.dest)
...
class TgzArchive(object):
    ...
    def is_unarchived(self, mode, owner, group):
        cmd = '%s %s -C "%s" --diff -%sf "%s"' % (self.cmd_path, self.extra_opts, self.dest, self.zipflag, self.src)
        rc, out, err = self.module.run_command(cmd)
        unarchived = (rc == 0)
        if not unarchived:
    ...
    def unarchive(self):
        cmd = '%s %s -x%sf "%s"' % (self.cmd_path, self.extra_opts, self.zipflag, self.src)
...
class TarArchive(TgzArchive):
    def __init__(self, src, dest, extra_opts, module):
        super(TarArchive, self).__init__(src, dest, extra_opts, module)
        self.zipflag = ''
...
class TarBzipArchive(TgzArchive):
    def __init__(self, src, dest, extra_opts, module):
        super(TarBzipArchive, self).__init__(src, dest, extra_opts, module)
        self.zipflag = 'j'
...
class TarXzArchive(TgzArchive):
    def __init__(self, src, dest, extra_opts, module):
        super(TarXzArchive, self).__init__(src, dest, extra_opts, module)
        self.zipflag = 'J'


# try handlers in order and return the one that works or bail if none work
def pick_handler(src, dest, extra_opts, module):
    handlers = [TgzArchive, ZipArchive, TarArchive, TarBzipArchive, TarXzArchive]
    for handler in handlers:
        obj = handler(src, dest, extra_opts, module)
...
def main():
    module = AnsibleModule(
        ...
        argument_spec = dict(
            ...
            extra_opts        = dict(required=False, default="", type='str'),
        ),
    ...
    handler = pick_handler(src, dest, extra_opts, module)
